...
//gets the last point from input array by extrapolating based on derivatives of points on an array
long int getnext(int *inp_arr, int point_num)
{
	int layers[100];//stores the layers of derivatives as we calculate them
	long int first_store;//stores the points extrapolated from the derivatives
	long int first_num_store[100];//stores the last point calculated from the derivatives
	...
	while(break_0){//calculates the derivatives layers
		first_num_store[num_of_layers] = layers[0];
		...
		++num_of_layers;
		printf(", %d %ld\n", num_of_layers, first_num_store[num_of_layers - 1]);

	}

	first_store = first_num_store[num_of_layers - 1];

	for(int i = num_of_layers - 2; i >= 0; --i)
		first_store = first_num_store[i] + first_store;
