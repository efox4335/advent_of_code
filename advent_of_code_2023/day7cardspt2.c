...
permote based on adding number of jokers to the largest block
...
int gethandtype(char *curr_hand)
{
	...
	int letter_dup = 0;
	int J_count = 0;
	//int biggest_ammount = 0;//keep track of what to premote
	...
	if(J_count > 0){
		printf("%d %s\n", J_count, curr_hand);
		switch (dup_card_num){//converts dupped card count to hand type to premote from
		case FOUR:
			return FIVE_OF_KIND;
		case THREE:
			if(J_count == 1)
