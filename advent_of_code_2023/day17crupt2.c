...
for part 2 added checks
	for turning it its gone 4 or more blocs
	for goinng straight if it has gone more then 10 blocks
	if it's at the finish if it's gone at least 4 blocks in a straight line
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
struct node node_list[100][4][200][200];//line place, call_direction, row, col
...
int find_end(void)
{
	for(int i = 0; i < open_node_number; ++i)
		...
		open_node_array[i].line_place >= 3)
...
void add_node(int line_place, int call_direction, int row, int col, int dist_from_start)
{
	...
	if(row < 0 || col < 0 || row >= map_hight || col >= map_length || line_place >= 10 || node_list[line_place][call_direction][row][col].seen == 1){
		return;
	}
