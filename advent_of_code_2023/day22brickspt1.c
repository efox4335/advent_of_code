...
//for each brick check if all bricks above it are suppoted by another brick if this is true for all bricks ++brick_could_remove_count
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main(void)
{
	...
	for(int i = 0; i < brick_num; ++i){
		total_removiable_bricks += can_remove(&brick_array[i]);
	}
...
int can_remove(brick *brick_ptr)
{
