...
pub fn solver() {
    let mut part = Part::Ranges;

    let mut foods: Vec<RangeKey> = Vec::new();
    ...
    for line in io::stdin().lines() {
        ...
        match part {
            ...
            Part::Foods => foods.push(RangeKey::Food(line.parse().unwrap())),
        }
    }

    let fresh_ids = foods.iter().fold(0, |acc, food_id| {
        if ranges.contains(food_id) {
            return acc + 1;
        }
